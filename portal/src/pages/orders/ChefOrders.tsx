import { useFrappeAuth, useFrappeDocTypeEventListener, useFrappeDocumentEventListener, useFrappeGetCall, useFrappePostCall, useFrappeUpdateDoc } from "frappe-react-sdk";
...
const ChefOrders = () => {
  ...
  const { data: result, mutate } = useFrappeGetCall("excel_restaurant_pos.api.item.get_chef_order_list", ['*']);
  ...
  const {call:markAsReady,loading,error}=useFrappePostCall("excel_restaurant_pos.api.item.make_as_ready_item")
              ...
              return (
                <>
                  {order?.item_list?.map((item, index) => (
                    <tr key={`${order.name}-${index}`} className={`${bgColor} hover:bg-gray-50`}>
                      {index === 0 ? (
                        <>
                      ...
                      <td className="px-4 py-2 border-b">{item?.is_ready ? "Ready" : <><button onClick={()=> handlemarkItemReady(order?.name,item?.name)}  className="w-full bg-primaryColor text-white py-1.5 px-4 rounded">Mark As Ready</button></>}</td>
                      {index === 0 ? (
                 ...
                 {
                  order?.remarks && <tr className={`bg-white`}>
                  <td colSpan={11} className="h-5"> Instructions:  {order?.remarks ? order?.remarks : "Not found"}</td>
                </tr>
                 }
