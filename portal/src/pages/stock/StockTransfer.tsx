...
export default function StockTransfer() {
  ...
  const [targetWarehouseSearch, setTargetWarehouseSearch] = useState("");
  const debouncedTargetWarehouseSearch = useDebounce(
    targetWarehouseSearch,
    300
  );

  const { createDoc, loading, error } = useFrappeCreateDoc<any>();

  const getSourceWarehouseDropdownList = () => {
    return useFrappeGetDocList("Warehouse", {
      ...
      filters: debouncedSearch
        ? ([["name", "like", `%${debouncedSearch}%`]] as any[])
        : undefined,
    });
  };

  const getTargetWarehouseDropdownList = () => {
    return useFrappeGetDocList("Warehouse", {
      fields: ["name"],
      orderBy: {
        field: "name",
      ...
      filters: debouncedTargetWarehouseSearch
        ? ([["name", "like", `%${debouncedTargetWarehouseSearch}%`]] as any[])
        : undefined,
    });
  };
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!sourceWarehouse || !targetWarehouse) {
      setMessage("Please select source and target warehouse.");
      return;
    }
    if (sourceWarehouse === targetWarehouse) {
      setMessage("Source and target warehouse cannot be the same.");
      return;
    }
    try {
      const items = rows
      ...
      // message handled by error state
    }
  };

  return (
    <div className="p-4 md:p-6">
      <h1 className="text-xl font-semibold mb-4">Stock Transfer</h1>
      <form onSubmit={onSubmit} className="space-y-4 max-w-4xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <SearchSelect
              options={
                getSourceWarehouseDropdownList()?.data?.map((w: any) => w.name) ?? []
              }
          ...
          <div>
            ...
            <SearchSelect
              options={
                getTargetWarehouseDropdownList()?.data?.map((w: any) => w.name) ?? []
              }
              value={targetWarehouse}
              onChange={setTargetWarehouse}
              searchText={targetWarehouseSearch}
              onSearchTextChange={setTargetWarehouseSearch}
              // disabled={isLoadingTargetWarehouses}
              placeholder="Search warehouses..."
            />
          </div>
        </div>
  ...
  return (
    <tr className="border-t">
      <td className="p-2">
        <SearchSelect
          options={
            getItemDropdownList()?.data?.map((i: any) => i.name) ?? []
          }
          value={row.item_code}
          onChange={(val) =>
            setRows((prev) =>
