...
from excel_restaurant_pos.api.item import make_as_create_recipe_item,make_as_unready_recipe_item

def create_work_order(bom_id=None,qty=None,source_warehouse=None,wip_warehouse=None,target_warehouse=None):
...
def make_stock_entry_custom(work_order_id, purpose="Manufacture", qty=5,target_warehouse="Finished Goods - ETL"):
    ...
    if isinstance(stock_entry_dict, dict):
        ...
        stock_entry.save()
        stock_entry.submit()
        print(f"Stock Entry {stock_entry.name} created successfully for purpose: {purpose}")
        return stock_entry
    else:
...
def create_process_log(bom_id, item_code, log_message, status="Success"):
    ...
    try:
        ...
        log_doc.save()
        frappe.db.commit()  # Commit log separately to ensure it's saved even if main process fails
        return log_doc
    except Exception as log_error:
        print(f"Error creating process log: {str(log_error)}")

def process(bom_id=None,qty=None,source_warehouse=None,wip_warehouse=None,target_warehouse=None,order_id=None):
    """
    ...
    try:
        ...
        item_code = item_code
        log_messages.append(f"[{datetime.now()}] BOM {bom_id} found for item {bom_doc.item}")
        
        # Step 1: Create Work Order
        log_messages.append(f"[{datetime.now()}] Creating work order...")
        work_order_doc = create_work_order(bom_id,flt(qty),source_warehouse,wip_warehouse,target_warehouse)
        log_messages.append(f"[{datetime.now()}] Work Order {work_order_doc.name} created successfully")
        
        # Step 2: Create Material Transfer Stock Entry
        log_messages.append(f"[{datetime.now()}] Creating material transfer stock entry...")
        ...
        log_messages.append(f"[{datetime.now()}] Material Transfer Stock Entry {material_transfer_entry.name} created successfully")
        ...
        log_messages.append(f"[{datetime.now()}] Manufacture Stock Entry {manufacture_entry.name} created successfully")
        ...
        if order_id:
            ...
            log_messages.append(f"[{datetime.now()}] Product {item_code} made as ready for order {order_id}")
        # Create success log
        final_log = "\n".join(log_messages)
        ...
        if order_id:
            ...
            log_messages.append(f"[{datetime.now()}] Product {item_code} made as unready for order {order_id}")
        ...
        if work_order_doc:
            log_messages.append(f"[{datetime.now()}] Work Order {work_order_doc.name} was created but rolled back")
        if material_transfer_entry:
            log_messages.append(f"[{datetime.now()}] Material Transfer Entry {material_transfer_entry.name} was created but rolled back")
        if manufacture_entry:
            log_messages.append(f"[{datetime.now()}] Manufacture Entry {manufacture_entry.name} was created but rolled back")
            
        log_messages.append(f"[{datetime.now()}] All changes have been rolled back due to error")
...
def run_bom_process(bom_id=None,qty=None,order_id=None):
    ...
    frappe.enqueue(process,queue="long",bom_id=bom_id,qty=flt(qty),source_warehouse=get_warehouse_config.source_warehouse,wip_warehouse=get_warehouse_config.wip_warehouse,target_warehouse=get_warehouse_config.target_warehouse,order_id=order_id)
